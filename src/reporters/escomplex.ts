...
const escomplexReporter = (source: string): CodehawkComplexityResult | null => {
  ...
  const vocab =
    report.aggregate.halstead.vocabulary < 2
      ? 1
  ...
  const absoluteCodehawkScore =
    171 - 5.2 * Math.log(vocab) - 0.23 * cyclo - 16.2 * Math.log(factor)
  const codehawkScore = (absoluteCodehawkScore * 100) / 171

  // Never minus
  trimmed.codehawkScore = codehawkScore > 0 ? codehawkScore : 0
